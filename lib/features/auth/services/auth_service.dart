...
class AuthService {
  ...
  Future<UserModel?> login(String username, String password) async {
    try {
      final baseUrl =
          await PosApiService().getBaseUrl() ?? 'https://api.namecheap.work';
          ...
          if (data['user'] != null) {
            ...
            try {
              ...
              if (firebasePassword.length < 6) {
                firebasePassword = firebasePassword.padRight(
                    6, '0'); // Firebase requires >= 6 chars
              }
            ...
            switch (roleStr) {
              ...
              default:
                role = UserRole
                    .requester; // CASHIER = พนักงานหน้าร้าน = requester ใน S-Link
                break;
            }
            ...
            final user = UserModel(
              ...
              name:
                  (u['employee_name'] ?? u['username'] ?? 'Unknown').toString(),
              role: role,
            );
        ...
        if (offUser != null &&
          ...
          try {
            await FirebaseAuth.instance.signInWithEmailAndPassword(
                email: '$username@s-link.local', password: password);
          } catch (_) {}
          ...
          if (parts.length == 3) {
            ...
            final roleStr =
                (payload['role'] ?? 'CASHIER').toString().toUpperCase();
            UserRole role;
            switch (roleStr) {
              case 'ADMIN':
                role = UserRole.admin;
