...
class _ShortageLogsViewState extends State<ShortageLogsView> {
  ...
  bool _canDelete(AuthenticationProvider authProvider) {
    return authProvider.canManageStockAlerts;
  }
  ...
  Future<void> _deleteAlertInstant(
      BuildContext context, dynamic alertId) async {
    try {
      await Provider.of<AlertLogProvider>(context, listen: false)
          .markAsDone(alertId);
      if (context.mounted) {
  ...
  void _confirmDeleteAlert(
      BuildContext context, dynamic alertId, String alertName) {
    showDialog(
      context: context,
